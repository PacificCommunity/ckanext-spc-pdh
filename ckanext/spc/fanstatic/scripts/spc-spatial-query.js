 ...
 */
this.ckan.module('spc-spatial-query', function ($, _) {
  ...
  return {
    ...
    initialize: function () {
      ...
      $('.show-filters').on('click', function() {
        ckan.pubsub.publish('spatial_query_center_map');
        setTimeout(function() {
          ckan.pubsub.publish('spatial_query_center_map');
        }, 0);
      });
    ...
    _onReady: function() {
      ...
      map.on('draw:created', function (e) {
        ...
        if(!$('.apply:visible', buttons).length) {
          $('.apply', buttons).trigger('click');
        }
      });
      ...
      ckan.pubsub.subscribe('spatial_query_center_map', function() {
        resetMap();
        setPreviousExtent();
      });
      // Reset map view
