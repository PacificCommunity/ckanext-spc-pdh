...
this.ckan.module('spatial-select', function ($, _) {
  ...
  return {
	...
	if (features[i].geometry.coordinates) {
	  ...
	  features[i].geometry.features.forEach(function(f){ features.push(f); });
	}
      }
      return result;
    },
	...
	try {
	  $('#formatted-spatial').text(JSON.stringify(JSON.parse(e.target.value), null, 2));
	  setExistingBBBox();
	} catch (e) {
	}
      });
	...
	draw: {
	  undo: true,
          polyline: false,
          polygon: {shapeOptions: module.options.style, allowIntersection: false},
          circle: false,
          marker: false,
	...
	var coordinates = module.flattenToCoordinates(editableLayers.toGeoJSON());
